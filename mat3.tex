...
\begin{gather*}
    ...
    \text{Naravna parameterizacija: } \\ x'(s)^2 + y'(s)^2 + z'(s)^2 = 1 \\
    \text{paramet. daljice: } \vec{r}(t) = r _A * t(r _B - r _ A)\\
...
\begin{gather*}
    \textbf{\scriptsize S. polje:}
        ...
        dS = \sqrt{1+z_x+z_y} \, dx \, dy. \quad f = f(r(x,y))\\
        [\vec{n} = (-z _x, -z _y, 1), \quad z = z(x,y) ]\\
    \textbf{\scriptsize V. polje:}
        \iint_S \mathbf{V} \cdot d\mathbf{S} =\\= \iint_S Pdydz+Qdzdx+Rdxdy =\\= \iint _S \mathbf V \cdot \vec{v} \text{ } dS = \iint_D [\mathbf{V}, \mathbf{r}_u, \mathbf{r}_v] \, du \, dv \\
...
\begin{gather*}
    \textcolor{black}{Greenov, Stokesov, Gaussov: }\\
     \oint_C (P \, dx + Q \, dy) = \iint_D \left(\frac{\partial Q}{\partial x} - \frac{\partial P}{\partial y}\right) dxdy\\ 
    \text{ \tiny C - rob pozitivno orientiranega} \circlearrowleft \text{\tiny sklenjenega območja D } \\
    \oint_C \mathbf{V} \cdot d\mathbf{r} = \iint_S rot \mathbf{V} \cdot d\mathbf{S} = \\= \iint_D (rot \vec{V},\vec{r_u},\vec{r_v})du \, dv\\
    \text{ \tiny C je rob površine S, in D je projekcija na ravnivo}\\
    \iint_S \vec{\mathbf{V}} \cdot d\mathbf{S} = \iiint_W div \vec{\mathbf{V}} \, dx \, dy \, dz\\
    \text{ \tiny S - zaprt površina, ki je rob telese W, usmerjena navzven} \\
    \text{Gauss: } \oiint_S  \vec{v} \cdot d\mathbf{S} = \iiint_V \text{div}\,\vec{v}\,dx\,dy\,dz; \\ 
    \text{vol}(V) = \iiint_V dx\,dy\,dz = \frac{1}{3}\oiint_S(x,y,z) \cdot d\mathbf{\vec{S}}\\
    \oiint_S \frac{\partial f}{\partial v} dS = \iiint_V \Delta f \, dx\,dy\,dz; \\ 
    \oiint_S \vec{v} \cdot d\mathbf{S} = 0 \text{ če je polje solenoidalno} \\
    \text{Stokes: } \oint_{C = \partial S} \vec{v} \cdot d\vec{r} = \iint_S rot \, \vec{v} \cdot d\mathbf{S}\\
    \oiint_S rot \, \vec{v} \cdot d\mathbf{S} = 0 \text{ če je polje irrotacionalno}
\end{gather*}

\section*{Kompleksna analiza}
\subsection*{Analitičnost in odvedljivost in harmoničnost}
\begin{gather*}
    \text{Analitična funkcija: } f(z) = u(x,y) + iv(x,y) \\
    \text{C-R pogoj: } u_x = v_y \quad u_y = -v_x \quad u-x = \frac{\partial u}{\partial x} \\
    u = \int v _y \, dx = - \int v _x \, dy + C; \quad \text{\small "je Analitična"}\\
    \text{Pogoj Harmoničnosti: }\\ 
    \nabla^2 u = \nabla^2 v = 0, \quad w _{xx} + w _{yy} = 0
\end{gather*}

\subsection*{Elementarne komp. fun.}
\begin{gather*}
    z = x + iy; \quad |z| = \sqrt{x^2 + y^2}; \quad\\
    \text{Polarni kot: } \arg z = \arctan\left(\frac{y}{x}\right) \\
...
\subsection*{lastnosti komp. funkcij}
\begin{gather*}
    e^{i\phi} = \cos \phi + i \sin \phi, \quad z = x+iy \\
    |e^{iy}| = 1; \, z = |z|e^{i\phi}, \, z^\alpha = e^{\alpha \ln z} \\
...
%         \qquad a_n = \frac{1}{2\pi i} \oint_C \frac{f(z)}{(z - z_0)^{n+1}} dz
% \end{gather*}

% \subsection*{Formule kompleksne analize in rezidumi}
% \begin{gather*}
...
\subsection*{Trigonometrske relacije}
\begin{gather*}
    \sin^2 x + \cos^2 x = 1\\ 
